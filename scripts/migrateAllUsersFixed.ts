...
async function migrateAllUsersFixed() {
  try {
    console.log('🚀 MIGRATING ALL USERS TO NEW STRUCTURE (FIXED VERSION)...');
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      try {
        ...
        const companyData = {
          ...
          // Metadata
          tags: ['migrated', 'all_fields_preserved', 'fixed_version'],
          notes: `Migrated from user ${userId} during fixed migration`,
        ...
        // Clean undefined values from company data
        const cleanedCompanyData = cleanUndefinedValues(companyData);
        await db.collection('companies').doc(companyId).set(cleanedCompanyData);
        ...
        // Clean undefined values from company user data
        const cleanedCompanyUserData = cleanUndefinedValues(companyUserData);
        await db.collection('companyUsers').doc(userId).set(cleanedCompanyUserData);
...
migrateAllUsersFixed()
  .then(() => {
    console.log('\n✅ Fixed user migration completed successfully!');
    process.exit(0);
  })
  .catch((error) => {
    console.error('\n❌ Fixed user migration failed:', error);
    process.exit(1);
  });
